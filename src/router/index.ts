...
export function createAppRouter(pinia: Pinia, entry: WindowEntry, history: RouterHistory = createWebHashHistory()) {
  const router = createRouter({ history, routes })
  const fallback = (): string => {
    if (entry.surface !== 'workspace') return entry.path
    try {
      // 兼容原来的 useStorage 格式，只记录主视图，不持久化 URL 或敏感参数。
      const saved = localStorage.getItem('miraihub:workspace-route')?.replace(/^"|"$/g, '')
      return isWorkspaceNav(saved) ? `/${saved}` : '/servers'
    } catch { return '/servers' }
  }

  router.beforeEach(to => {
    if (to.name === 'not-found' || to.path === '/' || to.meta.surface !== entry.surface)
      return { path: fallback(), replace: true }
    if (to.name === 'settings' && !SETTINGS_PAGES.some(page => page.id === to.params.section))
      return { name: 'settings', params: { section: 'general' }, replace: true }
    if (to.name === 'connection' && !['ssh', 'local', 'database'].includes(String(to.params.kind)))
  ...
  router.beforeResolve(async to => {
    ...
    const matches = (connection: { kind: string }) => to.name === 'databases'
      ? connection.kind === 'mysql' || connection.kind === 'postgresql'
      : connection.kind === 'ssh' || connection.kind === 'local'
    const id = typeof to.params.connectionId === 'string' ? to.params.connectionId : ''
    if (id) {
      ...
      const candidate = workspace.active && matches(workspace.active.connection)
        ? workspace.active : [...workspace.tabs].reverse().find(tab => matches(tab.connection))
      if (candidate)
        return { name: to.name, params: { connectionId: candidate.id }, replace: true }
    }
  })

  router.afterEach((to, _from, failure) => {
    if (failure) return
    if (typeof document !== 'undefined') document.title = `${to.meta.title ?? '工作区'} · MiraiHub`
    if (to.meta.nav) {
      try { localStorage.setItem('miraihub:workspace-route', to.meta.nav) } catch { /* 存储不可用时导航仍可用。 */ }
    }
  })
