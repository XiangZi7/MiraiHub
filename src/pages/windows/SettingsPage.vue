...
const isDirty = computed(() =>
  (Object.keys(draft) as SettingKey[]).some(
  ...
  >
    ...
    <div class="relative z-10 flex min-h-0 flex-1">
      ...
      <div class="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        ...
        >
          <AppButton size="sm" @click="resetToDefaults"> 重置为默认 </AppButton>
          <span v-if="isDirty" class="text-[10.5px] text-txt-4"
            >有未保存的修改</span
          >
          <div class="flex-1" />
          <AppButton size="sm" @click="closeDialog"> 取消 </AppButton>
