...
export function useWorkspaceNavigation() {
  ...
  async function selectNav(nav: NavId): Promise<void> { await router.push({ name: nav }) }
  ...
  async function followActiveTab(): Promise<void> {
    ...
    await router.replace(workspace.active ? connectionLocation(workspace.active.connection) : { name: activeNav.value, params: {} })
  }
