...
function readQueries(): SavedDatabaseQuery[] {
  ...
  try {
    ...
    return Array.isArray(value)
      ? value.filter(isSavedQuery).slice(0, MAX_QUERIES)
      : []
  } catch {
    return []
  }
...
export const useSavedQueriesStore = defineStore('saved-queries', () => {
  ...
  function persistQueries(): void {
    ...
    try {
      localStorage.setItem(
        QUERY_STORAGE_KEY,
        JSON.stringify(state.items.slice(0, MAX_QUERIES))
      )
    } catch {
  ...
  if (typeof window !== 'undefined')
    window.addEventListener('beforeunload', persistQueries)

  onScopeDispose(() => {
    if (persistenceTimer) clearTimeout(persistenceTimer)
    persistQueries()
    if (typeof window !== 'undefined')
      window.removeEventListener('beforeunload', persistQueries)
  })
  return { items: state.items, persistQueries }
})

if (import.meta.hot)
  import.meta.hot.accept(acceptHMRUpdate(useSavedQueriesStore, import.meta.hot))
