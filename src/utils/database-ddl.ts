...
export const MYSQL_COLUMN_TYPES = [
  ...
  "VARCHAR", "CHAR", "TEXT", "MEDIUMTEXT", "LONGTEXT", "BOOLEAN",
  "DATE", "DATETIME", "TIMESTAMP", "TIME", "JSON", "BINARY", "VARBINARY",
...
const INTEGER_TYPES = new Set(["BIGINT", "INT", "INTEGER", "SMALLINT", "TINYINT"]);
...
export function validateTableDraft({ kind, draft }: TableDesignerOptions): TableDesignerValidation {
  const errors: string[] = [];
  ...
  for (const [index, column] of draft.columns.entries()) {
    ...
    if (!columnTypes(kind).includes(column.dataType as never)) errors.push(`字段“${label}”的数据类型不受支持`);
    if (column.length && !/^\d+(?:\s*,\s*\d+)?$/u.test(column.length)) errors.push(`字段“${label}”的长度或精度格式不正确`);
    if (column.defaultValue && /;|--|\/\*|\*\//u.test(column.defaultValue)) errors.push(`字段“${label}”的默认值不能包含语句分隔符或注释`);
    if (column.autoIncrement) {
      autoIncrementCount += 1;
      if (!INTEGER_TYPES.has(column.dataType)) errors.push(`字段“${label}”只有整数类型才能自动递增`);
    }
  }
  if (kind === "mysql" && autoIncrementCount > 1) errors.push("MySQL 每张表只能有一个自动递增字段");
  ...
  for (const [index, item] of draft.indexes.entries()) {
    ...
    if (item.columns.some(column => !columnNames.has(column))) errors.push(`索引“${label}”包含不存在的字段`);
    if (item.kind === "fulltext" && kind !== "mysql") errors.push("全文索引当前只支持 MySQL");
  }
  ...
  for (const [index, foreignKey] of draft.foreignKeys.entries()) {
    ...
    else if (foreignKeyNames.has(normalized)) errors.push(`外键“${foreignKey.name}”重复`);
    else foreignKeyNames.add(normalized);
    if (!foreignKey.column || !columnNames.has(foreignKey.column)) errors.push(`外键“${label}”缺少有效的本地字段`);
    if (!foreignKey.referencedSchema.trim() || !foreignKey.referencedTable.trim() || !foreignKey.referencedColumn.trim()) errors.push(`外键“${label}”的引用目标不完整`);
    if (foreignKey.onDelete === "SET NULL" && !draft.columns.find(column => column.name === foreignKey.column)?.nullable) errors.push(`外键“${label}”使用 SET NULL 时本地字段必须允许 NULL`);
  }

  return { valid: errors.length === 0, errors };
}

export function buildCreateTableSql({ kind, draft }: TableDesignerOptions): string {
  ...
  const primaryColumns = draft.columns.filter(column => column.primaryKey).map(column => quoteIdentifier(column.name, kind));
  if (primaryColumns.length) definitions.push(`  PRIMARY KEY (${primaryColumns.join(", ")})`);

  if (kind === "mysql") {
    for (const index of draft.indexes) {
      const prefix = index.kind === "unique" ? "UNIQUE KEY" : index.kind === "fulltext" ? "FULLTEXT KEY" : "KEY";
      const method = index.kind === "fulltext" ? "" : ` USING ${index.method.toUpperCase()}`;
      definitions.push(`  ${prefix} ${quoteIdentifier(index.name, kind)} (${index.columns.map(column => quoteIdentifier(column, kind)).join(", ")})${method}`);
    }
  }
  ...
  if (kind === "postgresql") {
    for (const index of draft.indexes) {
      ...
      statements.push(`CREATE ${unique}INDEX ${quoteIdentifier(index.name, kind)} ON ${table} USING ${index.method} (${index.columns.map(column => quoteIdentifier(column, kind)).join(", ")});`);
    }
    if (draft.comment) statements.push(`COMMENT ON TABLE ${table} IS ${quoteLiteral(draft.comment, kind)};`);
    for (const column of draft.columns) {
      if (column.comment) statements.push(`COMMENT ON COLUMN ${table}.${quoteIdentifier(column.name, kind)} IS ${quoteLiteral(column.comment, kind)};`);
    }
  }
...
function renderColumn(column: TableDesignerColumn, kind: DatabaseKind): string {
  ...
  const identity = column.autoIncrement
    ? kind === "mysql" ? " AUTO_INCREMENT" : " GENERATED BY DEFAULT AS IDENTITY"
    : "";
  const nullable = column.primaryKey || !column.nullable ? " NOT NULL" : " NULL";
  const defaultValue = column.defaultValue && !column.autoIncrement ? ` DEFAULT ${column.defaultValue}` : "";
  const unique = column.unique && !column.primaryKey ? " UNIQUE" : "";
  const comment = kind === "mysql" && column.comment ? ` COMMENT ${quoteLiteral(column.comment, kind)}` : "";
...
function qualifiedName(schema: string, name: string, kind: DatabaseKind): string {
...
function quoteLiteral(value: string, kind: DatabaseKind): string {
  ...
  return kind === "mysql" ? `'${escaped.replaceAll("\\", "\\\\")}'` : `'${escaped}'`;
}
