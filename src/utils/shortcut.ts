...
export function parseShortcut(value: string): ParsedShortcut | null {
  ...
  const parsed: ParsedShortcut = { ctrl: false, alt: false, shift: false, meta: false, key: '' }
...
export function shortcutFromEvent(event: KeyboardEvent): string | null {
  ...
  if (!hasModifier && !STANDALONE_KEY.test(key))
    return null

  // Shift 单独配字母、数字没有意义（等同于输入大写字符）
  if (!event.ctrlKey && !event.altKey && !event.metaKey && event.shiftKey && key.length === 1)
    return null
...
export function formatShortcut(value: string): string {
  ...
  return serializeShortcut({ ...parsed, key: display[parsed.key] ?? parsed.key })
}
