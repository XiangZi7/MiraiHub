...
<template>
  <section class="approval" aria-label="操作审批">
    <div class="flex items-center gap-2 font-medium">
      <AppIcon name="lucide:shield-alert" :size="16" />{{
    ...
    <p class="mt-3 text-[11px] text-txt-3">执行目标（已锁定）</p>
    <p class="mt-1 break-all font-mono text-[12px] text-txt">{{ target }}</p>
    <p class="mt-3 text-[11px] text-txt-3">{{ approval.label }} · 完整内容</p>
    <pre class="command" dir="ltr">{{ approval.command }}</pre>
    <p class="text-[11px] text-txt-3">AI 提议的原因</p>
    <p
      class="mt-1 whitespace-pre-wrap break-words text-[12px] leading-relaxed text-txt-2"
    >
      {{ approval.reason }}
    </p>
    ...
    <label
      class="my-3 flex cursor-pointer items-start gap-2 text-[11px] text-txt-2"
      ><input
        v-model="reviewed"
        type="checkbox"
    ...
    <div class="flex flex-wrap gap-2">
      <AppButton size="sm" :disabled="busy" @click="emit('decide', false)"
        >拒绝并停止</AppButton
      ><AppButton
        size="sm"
        variant="danger"
