...
function rootEnabledItems(): HTMLButtonElement[] {
  return [
    ...(menu.value?.querySelectorAll<HTMLButtonElement>(
      ':scope > .app-context-menu-entry > [role="menuitem"]:not(:disabled)'
    ) ?? []),
  ]
}
...
async function positionMenu(): Promise<void> {
  ...
  const left = Math.max(
    padding,
    Math.min(props.x, window.innerWidth - width - padding)
  )
  const top = Math.max(
    padding,
    Math.min(props.y, window.innerHeight - height - padding)
  )
...
function handleKeydown(event: KeyboardEvent): void {
  ...
  if (event.key === 'ArrowDown')
    ...
    current?.dataset.hasChildren === 'true'
  ) {
    event.preventDefault()
    current.parentElement
      ?.querySelector<HTMLButtonElement>(
        '.app-context-submenu [role="menuitem"]:not(:disabled)'
      )
      ?.focus()
    return
          ...
          <div class="app-context-menu-entry">
            <button
              ...
              :class="[
                ...
                item.danger && 'app-context-menu-item-danger',
              ]"
            ...
            >
              <span
                :class="[
                  ...
                  item.iconTone && `app-context-menu-icon-${item.iconTone}`,
                ]"
                aria-hidden="true"
              >
                <AppIcon
              ...
              <kbd
                v-if="item.shortcut"
                class="app-context-menu-shortcut"
                >{{ item.shortcut }}</kbd
              >
              <AppIcon
            ...
            <div
              ...
              :class="[
                'app-context-submenu',
                submenuLeft && 'app-context-submenu-left',
              ]"
              role="menu"
              ...
              >
                ...
                <div class="app-context-menu-entry">
                  <button
                    ...
                    :class="[
                      'app-context-menu-item',
                      child.danger && 'app-context-menu-item-danger',
                    ]"
                  ...
                  >
                    <span
                      :class="[
                        'app-context-menu-icon',
                        child.iconTone &&
                          `app-context-menu-icon-${child.iconTone}`,
                      ]"
                      aria-hidden="true"
                    >
                      <AppIcon
...
.app-context-menu-scroll {
  max-height: calc(100dvh - 16px);
  overflow-y: auto;
  overscroll-behavior: contain;
}
.app-context-menu-entry {
  position: relative;
}
.app-context-submenu {
  position: absolute;
...
.app-context-submenu-left {
  right: calc(100% - 1px);
  left: auto;
}
.app-context-menu-entry:hover > .app-context-submenu,
.app-context-menu-entry:focus-within > .app-context-submenu {
  display: block;
}
...
.app-context-menu-item:focus-visible {
  box-shadow: inset 0 0 0 1px
    color-mix(in oklch, var(--color-violet) 42%, transparent);
}
...
.app-context-menu-icon-violet {
  color: var(--color-violet);
}
.app-context-menu-icon-blue {
  color: var(--color-blue);
}
.app-context-menu-icon-amber {
  color: var(--color-amber);
}
.app-context-menu-icon-danger {
  color: var(--color-danger);
}

.app-context-menu-shortcut {
  flex: none;
