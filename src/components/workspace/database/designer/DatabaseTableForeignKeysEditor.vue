...
const localColumnOptions = computed(() => props.columns.map(column => ({
  value: column.name,
  label: column.name || "未命名字段",
})));
const tableSelectOptions = computed(() => tableOptions.value.map(table => ({ value: table.key, label: table.key })));
const actionOptions = referentialActions.map(value => ({ value, label: value }));

function referencedColumnOptions(foreignKey: TableDesignerForeignKey): Array<{ value: string; label: string }> {
  return (props.referencedColumns[referenceKey(foreignKey)] ?? []).map(value => ({ value, label: value }));
}
...
<template>
  <section class="designer-section">
    <div class="designer-section-heading">
      ...
      <AppButton size="sm" @click="addForeignKey"><AppIcon name="lucide:plus" :size="11" />添加外键</AppButton>
    </div>
    ...
    <div class="foreign-key-list scroll-thin">
      <article v-for="foreignKey in modelValue" :key="foreignKey.id" class="foreign-key-card">
        <div class="foreign-key-heading">
          ...
          <label class="field-label">约束名<AppInput :model-value="foreignKey.name" monospace @update:model-value="updateForeignKey(foreignKey.id, { name: $event })" /></label>
          <IconButton icon="lucide:trash-2" :size="12" class="size-7 text-danger hover:text-danger" title="删除外键" @click="removeForeignKey(foreignKey.id)" />
        </div>

        <div class="reference-flow">
          <div class="field-label"><span>本地字段</span><AppSelect :model-value="foreignKey.column" label="本地字段" :options="localColumnOptions" placeholder="请选择字段" hide-label compact @update:model-value="updateForeignKey(foreignKey.id, { column: $event })" /></div>
          <span class="flow-arrow"><AppIcon name="lucide:arrow-right" :size="14" /></span>
          <div class="field-label"><span>引用表</span><AppSelect :model-value="referenceKey(foreignKey)" label="引用表" :options="tableSelectOptions" placeholder="请选择引用表" hide-label compact searchable @update:model-value="selectTable(foreignKey, $event)" /></div>
          <span class="flow-arrow"><AppIcon name="lucide:arrow-right" :size="14" /></span>
          <div class="field-label"><span>引用字段</span><AppSelect :model-value="foreignKey.referencedColumn" label="引用字段" :options="referencedColumnOptions(foreignKey)" :placeholder="loadingReference === referenceKey(foreignKey) ? '正在读取字段…' : '请选择字段'" hide-label compact :disabled="!foreignKey.referencedTable || loadingReference === referenceKey(foreignKey)" @update:model-value="updateForeignKey(foreignKey.id, { referencedColumn: $event })" /></div>
        </div>

        <div class="action-grid">
          <div class="field-label"><span>删除时</span><AppSelect :model-value="foreignKey.onDelete" label="删除时" :options="actionOptions" hide-label compact @update:model-value="updateForeignKey(foreignKey.id, { onDelete: $event as ReferentialAction })" /></div>
          <div class="field-label"><span>更新时</span><AppSelect :model-value="foreignKey.onUpdate" label="更新时" :options="actionOptions" hide-label compact @update:model-value="updateForeignKey(foreignKey.id, { onUpdate: $event as ReferentialAction })" /></div>
