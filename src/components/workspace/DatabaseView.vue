...
type NameActionMode = "create-database" | "rename-database" | "rename-object" | "save-query" | "create-saved-query" | "rename-saved-query";
...
const queryEditorRatio = useStorage<number>(`miraihub:database-query-split:${props.connection?.id ?? "default"}`, 58);
const querySplitStyle = computed(() => {
  const editorRatio = Math.min(85, Math.max(15, Number.isFinite(queryEditorRatio.value) ? queryEditorRatio.value : 58));
  return { gridTemplateRows: `${editorRatio}fr 10px ${100 - editorRatio}fr` };
});
const configuredDatabase = props.connection && isDatabaseConnection(props.connection) ? props.connection.settings.database : "";
const {
...
const savedQueriesById = new Map(savedQueries.value.map(query => [query.id, query]));
const restoredQueryTabs = restoredWorkspace.tabs.map((tab): QueryTab => {
  const saved = tab.savedQueryId ? savedQueriesById.get(tab.savedQueryId) : undefined;
  return {
    id: tab.id,
...
const initialQueryTabs = restoredQueryTabs.length ? restoredQueryTabs : [{
  id: "query-1",
  label: "Query 1",
...
const queryState = reactive({
  ...
  activeId: initialQueryTabs.some(tab => tab.id === restoredWorkspace.activeId) ? restoredWorkspace.activeId : initialQueryTabs[0]?.id ?? "query-1",
});
...
const activeDesigner = computed(() => activeTab.value?.kind === "table-designer" ? activeTab.value : undefined);
const selectedSavedQueryId = computed(() => activeQuery.value?.savedQueryId ?? "");
const relationTabs = computed(() => queryState.tabs.filter((tab): tab is ObjectTab =>
  tab.kind === "object" && (tab.object.kind === "table" || tab.object.kind === "view"),
));
const routineTabs = computed(() => queryState.tabs.filter((tab): tab is ObjectTab =>
  tab.kind === "object" && (tab.object.kind === "procedure" || tab.object.kind === "function"),
));
const designerTabs = computed(() => queryState.tabs.filter((tab): tab is TableDesignerTab => tab.kind === "table-designer"));
const activeSql = computed({
  ...
  set: (value: string) => {
    ...
    if (!tab) return;
    tab.sql = value;
    if (tab.savedQueryId) updateSavedQuery(tab.savedQueryId, { sql: value, database: tab.database });
  },
});
const persistedQueryTabs = computed<PersistedDatabaseQueryTab[]>(() => queryState.tabs.flatMap((tab) => tab.kind === "query" ? [{
  id: tab.id,
  label: tab.label,
  sql: tab.sql,
...
watch([persistedQueryTabs, lastActiveQueryId], () => void persistQueryWorkspaceSoon(), { deep: true, immediate: true });
...
const nameDialogDescription = computed(() => {
  ...
  if (nameDialog.mode === "save-query" || nameDialog.mode === "create-saved-query") return `${nameDialog.queryDatabase} · 保存后会显示在 Queries 中，并自动同步后续修改`;
  if (nameDialog.mode === "rename-saved-query") return `${nameDialog.targetSavedQuery?.database ?? nameDialog.queryDatabase} / Queries`;
...
const deleteTitle = computed(() => deleteDialog.savedQuery ? "删除已保存查询？" : deleteDialog.object ? `删除${objectKindLabel(deleteDialog.object.kind)}？` : "删除数据库？");
const deleteDescription = computed(() => {
  if (deleteDialog.savedQuery) return `将删除查询“${deleteDialog.savedQuery.name}”。已经打开的标签会转为临时草稿，SQL 内容不会丢失。`;
...
function addQueryTab(sql = "", targetDatabase = session.value?.database || configuredDatabase): QueryTab {
...
function openQuery(sql: string): void {
  const tab = activeQuery.value && !activeQuery.value.savedQueryId ? activeQuery.value : addQueryTab();
  tab.sql = sql;
...
async function openSavedQuery(query: SavedDatabaseQuery): Promise<void> {
  if (databaseKind.value === "mysql" && query.database && query.database !== session.value?.database) await changeDatabase(query.database);
  const id = `saved:${query.id}`;
  const existing = queryState.tabs.find(tab => tab.kind === "query" && tab.savedQueryId === query.id);
...
function saveActiveQuery(): void {
  ...
  showQueryNameDialog("save-query", tab.database || databaseName.value, tab.sql, tab);
}

function showQueryNameDialog(
  mode: Extract<NameActionMode, "save-query" | "create-saved-query" | "rename-saved-query">,
  ...
  nameDialog.targetQueryTabId = target && "kind" in target && target.kind === "query" ? target.id : "";
  ...
  nameDialog.initialValue = mode === "rename-saved-query"
    ? nameDialog.targetSavedQuery?.name ?? ""
    : mode === "save-query" && target && "label" in target && !/^Query \d+$/u.test(target.label) ? target.label : "";
  nameDialog.loading = false;
  nameDialog.open = true;
}

function duplicateSavedQuery(query: SavedDatabaseQuery): void {
  const duplicate = createSavedQuery(query.database, `${query.name} 副本`, query.sql);
  persistQueries();
...
function createObjectTemplate(schema: string, kind: DatabaseObjectKind): void {
  ...
  const placeholder = kind === "view" ? "new_view" : kind === "procedure" ? "new_procedure" : "new_function";
...
async function handleTableCreated(tabId: string, schema: string, name: string): Promise<void> {
  closeTab(tabId);
  await refreshObjects();
  const object = objects.value.find(candidate => candidate.kind === "table" && candidate.schema === schema && candidate.name === name);
  if (object) openObject(object);
  toast.success(object ? `表“${name}”已创建并打开` : `表“${schema}.${name}”已创建`);
}
...
async function submitNameAction(name: string): Promise<void> {
  ...
  try {
    ...
    if (nameDialog.mode === "save-query") {
      const tab = queryState.tabs.find(candidate => candidate.kind === "query" && candidate.id === nameDialog.targetQueryTabId);
      if (!tab || tab.kind !== "query") throw new Error("要保存的查询标签已关闭");
      ...
      const saved = createSavedQuery(nameDialog.queryDatabase, name, nameDialog.querySql || "SELECT 1;");
      persistQueries();
      await openSavedQuery(saved);
      successMessage = `查询“${saved.name}”已创建`;
    } else if (nameDialog.mode === "rename-saved-query" && nameDialog.targetSavedQuery) {
      ...
      for (const tab of queryState.tabs) {
        if (tab.kind === "query" && tab.savedQueryId === saved.id) tab.label = saved.name;
      }
      persistQueries();
    ...
    toast.error({ title: "操作失败", description: database.errorMessage(error) });
  } finally {
    nameDialog.loading = false;
  }
}
...
async function confirmDelete(): Promise<void> {
  ...
  if (targetSavedQuery) {
    ...
    for (const tab of queryState.tabs) {
      if (tab.kind !== "query" || tab.savedQueryId !== targetSavedQuery.id) continue;
    ...
    deleteDialog.savedQuery = null;
    toast.success(`已删除查询“${targetSavedQuery.name}”，打开的内容已保留为草稿`);
    return;
  }
  ...
  try {
    ...
    toast.error({ title: "删除失败", description: database.errorMessage(error) });
  }
}
...
async function runQuery(sqlOverride?: string): Promise<void> {
  ...
  if (databaseKind.value === "mysql" && queryDatabase && queryDatabase !== session.value?.database) await changeDatabase(queryDatabase);
  if (props.connection) {
...
async function changeDatabase(value: string): Promise<void> {
  ...
  for (const tab of [...queryState.tabs]) if (tab.kind === "object" || tab.kind === "table-designer") closeTab(tab.id);
...
<template>
  ...
  <div v-else class="pane flex-1 flex-row">
    <DatabaseObjectTree
      ...
      @create-saved-query="(targetDatabase) => showQueryNameDialog('create-saved-query', targetDatabase)"
      @open-saved-query="openSavedQuery"
      @duplicate-saved-query="duplicateSavedQuery"
      @rename-saved-query="(query) => showQueryNameDialog('rename-saved-query', query.database, query.sql, query)"
    ...
    <div class="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      <DatabaseConnectionState v-if="!connected" v-model:password="password" :status="status" :needs-password="needsPassword" @connect="connect" />
      <template v-else>
        <div v-show="Boolean(activeQuery)" class="query-workspace" :style="querySplitStyle">
          <SqlEditor ref="editor" v-model="activeSql" :disabled="queryLoading" :suggestions="sqlSuggestions" @run="runQuery" @save="saveActiveQuery" />
        ...
        <DatabaseTableView v-for="tab in relationTabs" v-show="queryState.activeId === tab.id" :key="tab.id" :session-id="sessionId" :object="tab.object" :initial-panel="tab.panel" :panel-nonce="tab.panelNonce" @query="openQuery" />
