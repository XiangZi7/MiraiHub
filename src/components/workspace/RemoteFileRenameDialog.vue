...
<template>
  <AppDialog title="重命名远端文件" description="只修改名称，文件仍保留在当前目录。" @close="emit('close')">
    <form @submit.prevent="submit">
      <label for="remote-file-name" class="block text-[11px] font-medium text-txt-2">新名称</label>
      <input
        id="remote-file-name"
        ref="input"
        v-model="value"
    ...
    <template #footer>
      <div class="flex-1" />
      <AppButton size="sm" @click="emit('close')">取消</AppButton>
      <AppButton size="sm" variant="primary" @click="submit">重命名</AppButton>
    </template>
  </AppDialog>
</template>
