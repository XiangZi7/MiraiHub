...
<template>
  <main class="remote-editor-window" :aria-busy="closing">
    <p v-if="error" role="alert" class="window-error">{{ error }}</p>
    <RemoteTextEditor
      v-if="target"
      ref="editor"
      v-bind="target"
    ...
    <div v-else class="window-loading">
      ...
      <AppButton :disabled="closing" @click="finish">关闭窗口</AppButton>
    </div>
  </main>
</template>
<style scoped>
