...
use super::*;
use std::{io::Write, path::{Path, PathBuf}, sync::Weak};
...
impl Record {
    fn from_run(run: &Run, cell: &Cell) -> Self {
        Self {
            summary: Summary {
                id: run.history.id.clone(), title: run.history.title.clone(),
                model: run.config.model.clone(), provider: run.config.base_url.clone(),
                created_at: run.history.created_at, updated_at: now(),
            },
            scope: run.history.scope.clone(), target: run.target_label.clone(),
            ...
            messages: run.messages.iter().filter(|m| m["role"] != "system").cloned().collect(),
        }
    }
    pub fn identity(&self) -> Identity {
        Identity { id: self.summary.id.clone(), title: self.summary.title.clone(), scope: self.scope.clone(), created_at: self.summary.created_at }
    }
    pub fn snapshot(self) -> Snapshot {
        Snapshot {
            id: String::new(), conversation_id: self.summary.id,
            target: self.target, model: self.summary.model, provider: self.summary.provider,
            status: match self.status.as_str() { "running" | "approval" => "cancelled".into(), _ => self.status },
            entries: self.entries, approval: None, save_error: None,
        }
    }
}
...
fn directory(app: &AppHandle) -> AppResult<PathBuf> {
    Ok(app.path().app_data_dir().map_err(|_| AppError::internal("无法定位聊天记录目录"))?.join("ai-conversations"))
}
...
fn read(dir: &Path, id: &str, scope: &str) -> AppResult<Record> {
    ...
    let record: Record = serde_json::from_slice(&bytes).map_err(|_| AppError::invalid_input("聊天记录已损坏"))?;
...
impl Store {
    pub fn register(&mut self, conversation_id: &str, cell: &Arc<Cell>) {
        self.owners.insert(conversation_id.into(), Arc::downgrade(cell));
    }
    pub fn save(&self, app: &AppHandle, run: &Run, cell: &Cell) -> AppResult<()> {
        if self.owners.get(&run.history.id).and_then(Weak::upgrade).is_some_and(|owner| std::ptr::eq(owner.as_ref(), cell)) {
            write(&directory(app)?, &Record::from_run(run, cell))?;
        }
        Ok(())
    }
    pub fn resume(&self, app: &AppHandle, conversation_id: &str, scope: &str) -> AppResult<Record> {
        ...
        if let Some(owner) = self.owners.get(conversation_id).and_then(Weak::upgrade) {
            let run = owner.run.try_lock().map_err(|_| AppError::invalid_input("上一轮操作仍在结束中，请稍后继续此会话"))?;
            if !owner.cancelled.load(Ordering::SeqCst) && matches!(run.status.as_str(), "running" | "approval") {
                return Err(AppError::invalid_input("此会话仍在处理中，请先停止当前操作"));
            }
...
pub(super) fn resume_messages(messages: Vec<Value>) -> Vec<Value> {
    ...
    for message in messages {
        ...
        if let Some(calls) = message["tool_calls"].as_array() {
            pending.extend(calls.iter().filter_map(|call| call["id"].as_str().map(String::from)));
        }
        result.push(message);
    }
    close_pending(&mut result, &mut pending);
...
pub(super) async fn scope(app: &AppHandle, target: &Target) -> AppResult<String> {
    ...
    let engine = if target.kind == "database" { format!("{:?}", app.state::<db::DatabaseManager>().describe_session(&target.session_id).await?.kind) } else { String::new() };
    Ok(json!([target.kind, label, engine]).to_string())
}

#[tauri::command]
pub async fn ai_list_conversations(window: WebviewWindow, app: AppHandle, state: State<'_, AgentManager>, target: Target) -> AppResult<Vec<Summary>> {
    guard(&window, false)?;
    ...
    for file in files {
        let path = file?.path();
        if path.extension().and_then(|v| v.to_str()) != Some("bin") { continue; }
        let bytes = config::protect(&std::fs::read(&path)?, false)?;
        let record: Record = serde_json::from_slice(&bytes).map_err(|_| AppError::invalid_input("聊天记录已损坏，无法加载历史列表"))?;
        if record.scope == scope { summaries.push(record.summary); }
    }
    summaries.sort_by(|a, b| b.updated_at.cmp(&a.updated_at).then_with(|| a.id.cmp(&b.id)));
    Ok(summaries)
}
#[tauri::command]
pub async fn ai_open_conversation(window: WebviewWindow, app: AppHandle, state: State<'_, AgentManager>, target: Target, conversation_id: String) -> AppResult<Snapshot> {
    guard(&window, false)?;
    let scope = scope(&app, &target).await?;
    let _store = state.history.lock().await;
...
#[tauri::command]
pub async fn ai_delete_conversation(window: WebviewWindow, app: AppHandle, state: State<'_, AgentManager>, target: Target, conversation_id: String) -> AppResult<()> {
    guard(&window, false)?;
    let scope = scope(&app, &target).await?;
    ...
    if let Some(owner) = store.owners.remove(&conversation_id).and_then(|owner| owner.upgrade()) {
        owner.cancelled.store(true, Ordering::SeqCst);
    }
    Ok(())
}
