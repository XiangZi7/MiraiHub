...
impl ApprovalMode {
    ...
    pub fn instruction(self) -> &'static str {
        match self {
            ...
            Self::Full => "The user selected Full access for this turn on the current connection: all provided tools, including custom shell, SQL, Redis and external MCP tools, run without a separate approval. Call tools directly to complete the user's requested task on the bound target. The propose_ prefix is a tool name, not a requirement to ask permission. Do not stop to request confirmation in prose or ask the user to approve tool calls. Ask only for missing information required to perform the task. Do not claim success without a tool result.",
        }
    }
}
