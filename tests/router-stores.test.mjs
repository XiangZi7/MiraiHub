...
const saved = ["a", "b", "db"].map((id) => ({
  id,
  name: id,
  kind: id === "db" ? "mysql" : "ssh",
  group: "",
  createdAt: 1,
  settings: {},
}));
...
const load = sourceLoader({
  "@/api/settings": settingsApiUrl,
  "@/api/connections": connectionsApiUrl,
});
...
test("兼容 Rust 子窗口 query，原生 label 的窗口身份优先且不泄露编辑目标", () => {
  assert.deepEqual(
    resolveWindowEntry(
      "?window=connection&type=database&connectionId=db-1",
      "",
      "connection",
    ),
    { surface: "connection", path: "/connection/database?connectionId=db-1" },
  );
  assert.deepEqual(resolveWindowEntry("?window=settings", "", "main"), {
    surface: "workspace",
    path: "/servers",
  });
  assert.deepEqual(resolveWindowEntry("", "#/settings/ai"), {
    surface: "settings",
...
test("无效连接、错误类型与未知地址不会打开错误标签或白屏", async () => {
  ...
  try {
    for (const path of ["/servers/missing", "/servers/db", "/databases/a"]) {
      ...
      assert.ok(
        ["/servers", "/databases"].includes(router.currentRoute.value.fullPath),
      );
    }
...
test("存储刷新后历史导航不替换活动连接快照，不触发隐式重连", async () => {
  const { router, workspace, pinia, dispose } = fixture();
  const api = await import(connectionsApiUrl);
  try {
    await router.push("/databases/db");
    const snapshot = workspace.active.connection;
    workspace.setStatus("db", "connected", "live-session");
    await router.push("/recent");
    api.replace(saved.map((item) => ({ ...item, lastUsedAt: 123 })));
    await useConnectionsStore(pinia).initialize();
    await router.push("/databases/db");
    assert.equal(workspace.active.connection, snapshot);
    assert.equal(workspace.active.sessionId, "live-session");
  } finally {
    dispose();
    api.replace(saved);
  }
...
test("工作区 Store 隔离、恢复去重和关闭后的持久化不包含会话信息", async () => {
  ...
  storage.set(
    "miraihub:workspace-tabs",
    JSON.stringify({ ids: ["a", "a", "deleted", "db"], activeId: "db" }),
  );
...
test("按路由名缓存页面保留草稿，关闭后台标签会卸载对应会话", async () => {
  ...
  const Servers = defineComponent({
    setup() {
      draft = ref("SELECT 1");
      return () =>
        h(
          "div",
          ids.value.map((id) => h(Session, { key: id })),
        );
    },
  });
  ...
  const app = renderer.createApp({
    render: () =>
      h(KeepAlive, null, {
        default: () =>
          h(current.value === "servers" ? Servers : Recent, {
            key: current.value,
          }),
      }),
  });
...
test("本地存储被禁用时仍可恢复空工作区和导航", async () => {
  const { router, workspace, dispose } = fixture();
...
test("已保存查询目录迁入 Store 后仍按连接隔离，销毁时刷新待保存内容", async () => {
  storage.clear();
  const pinia = createPinia();
  setActivePinia(pinia);
  const { useSavedDatabaseQueries } = await load(
    "src/composables/useSavedDatabaseQueries.ts",
  );
  try {
    const a = useSavedDatabaseQueries("a");
    const b = useSavedDatabaseQueries("b");
    const first = a.createQuery("preview", "示例", "SELECT 1");
    b.createQuery("preview", "示例", "SELECT 2");
    assert.equal(a.queries.value.length, 1);
    assert.equal(b.queries.value.length, 1);
    assert.equal(
      b.updateQuery(first.id, { sql: "cross connection" }),
      undefined,
    );
    a.updateQuery(first.id, { sql: "SELECT 42" });
    await nextTick();
  } finally {
    disposePinia(pinia);
  }
  const savedQueries = JSON.parse(
    storage.get("miraihub.database-saved-queries.v1"),
  );
  assert.equal(
    savedQueries.find((query) => query.connectionId === "a").sql,
    "SELECT 42",
  );
  assert.equal(
    savedQueries.find((query) => query.connectionId === "b").sql,
    "SELECT 2",
  );
});
