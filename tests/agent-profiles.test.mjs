...
const load = sourceLoader({ '@/api/agent': mockUrl, '@/utils/window': dataModule('export const IS_TAURI = true') })
...
function fixture(composable) {
  api.settings = { activeId:'a', profiles: ['a','b'].map(id => ({ id, name:id, enabled:true, apiFormat:'openai', baseUrl:'https://same.example/v1', model:id+'-model', hasApiKey:true })) }
  api.handlers.clear(); api.deferred = null; api.saved = null
  let state
  ...
  const app = renderer.createApp({setup() { state = composable(); return () => null }})
  app.mount({})
  return { app, state }
}
test('profile drafts survive selection changes and saving clears only the saved draft key', async () => {
  ...
  try {
    ...
    state.preset.value = 'claude'; state.add()
    assert.equal(state.draft.value.apiFormat, 'anthropic')
    state.draft.value.model = 'claude-test'; state.draft.value.apiKey = 'new-test-key'
    await state.save()
...
test('panel activation updates the selected profile and late metadata cannot undo it', async () => {
  ...
  } finally { app.unmount(); api.deferred = null }
})
